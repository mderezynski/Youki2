...
#include <uxn/patl/impl/prefix.hpp>
#include <uxn/patl/impl/preorder_iterator.hpp>
...
public:
    ...
    {
        this_t vtx(*this);
        if (const_vertex_generic::compact())
            vtx.descend<0>();
        else
            vtx.toggle();
...
public:
    ...
    operator algorithm&()
    {
        return const_vertex_generic<Algorithm>::pal_;
    }
    ...
    {
        this_t vtx(*this);
        if (const_vertex_generic<Algorithm>::pal_.get_q())
            vtx.template move<1>();
        else
            vtx.toggle();
        return postorder_iterator(vtx);
    ...
    {
        const node_type *q = const_vertex_generic<Algorithm>::pal_.get_q();
