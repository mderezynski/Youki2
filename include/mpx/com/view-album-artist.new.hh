...
namespace Artist
{
        ...
        struct DataModelFilter : public DataModel
        {
                ...
                )
                {
                    ...
                    get<0>(row) = (boost::format(_("<b>%lld %s</b>")) % model_size % ((model_size > 1) ? _("Artists") : _("Artist"))).str() ;
                }
        };
        ...
        class Class : public Gtk::DrawingArea
        {
			...
			m_prop_vadj.get_value()->set_page_increment( 10 ) ;
                    }
                }
...
*/
		    ...
		    cairo->fill() ;
                    ...
                    RowRowMapping_t::const_iterator iter = m_model->row_iter( row ) ;

                    for( std::size_t n = 0 ; n < limit ; n += 2 ) 
                    {
                            GdkRectangle r ;
                            r.x       = inner_pad ;
                            r.y       = inner_pad + m_height__row*n ;
                            r.width   = a.get_width() - 2*inner_pad ; 
                            r.height  = m_height__row - 2*inner_pad ;
                    ...
                    for( std::size_t n = 0 ; n < limit && (row+n) < m_model->size() ; ++n )
                    {
			xpos = 0 ;
...
/*
		    ...
		    int pos = m_prop_vadj.get_value()->get_value() * m_height__row ;
		    int pgs = m_prop_vadj.get_value()->get_page_size() * m_height__row ;
		    int upp = m_prop_vadj.get_value()->get_upper() * m_height__row ;
