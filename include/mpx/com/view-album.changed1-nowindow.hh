...
*/
		...
		)
		{
			...
			cairo->set_line_width( (!album->type.empty()) ? 1.75 : 0.75 ) ;
			cairo->stroke() ;
		    }
...
/*
		    // Track count in the database differs from apparent disc track count
...
*/
			...
			{
			    album->surfacecache = render_icon( disc, album, widget, m_rt_viewmode ) ;
			}
		    ...
		    font_desc[L1].set_stretch( Pango::STRETCH_EXTRA_CONDENSED ) ;
			    ...
			    if( m_show_year_label )
			    {
				    ...
				    {
					if( is_selected )
						layout[L3]->set_markup((boost::format("<span color='#e0e0e0'>%s</span>") % Glib::Markup::escape_text(album->year.substr(0,4).c_str()).c_str()).str()) ;
					else
						layout[L3]->set_markup((boost::format("<span color='#909090'>%s</span>") % Glib::Markup::escape_text(album->year.substr(0,4).c_str()).c_str()).str()) ;
				    }
				    else
				    {
					if( is_selected )
						layout[L3]->set_markup((boost::format("<span color='#e0e0e0'>%s %s</span>") % Glib::Markup::escape_text(album->year.substr(0,4).c_str()).c_str() % Glib::Markup::escape_text(album->label).c_str()).str()) ;
					else
						layout[L3]->set_markup((boost::format("<span color='#909090'>%s %s</span>") % Glib::Markup::escape_text(album->year.substr(0,4).c_str()).c_str() % Glib::Markup::escape_text(album->label).c_str()).str()) ;
				    }
			    ...
			    {
                            	layout[L1]->set_text( (boost::format(("%lld %s")) % album_count % ( (album_count > 1) ? ("Albums") : ("Album"))).str()) ;
			    }
		            else
			    {
		...
		bool				    m_show_year_label ;
		    ...
		    std::size_t w = a.get_width(), h = a.get_height() ;
		    ...
		    if( row+n == m_model->size() -1 )
		    {
			c = MPX::CairoCorners::CORNERS(12) ;
		    }
		    else
		    {
			c = MPX::CairoCorners::CORNERS(0) ;
		    }
