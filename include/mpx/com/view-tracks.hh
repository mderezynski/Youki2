...
namespace Tracks
{
	struct ModelData_t
	{
	    ...
	    boost::optional<guint> sort_order ;
	} ;
...
*/
		...
		get_total_time()
		{
		    ...
		    {
			MPX::Track_sp t = (**i)->TrackSp ; 
			total += get<guint>((*t)[ATTRIBUTE_TIME].get()) ;
		    }
        ...
        class Column
        {
			...
			layout_s->set_text((boost::format("%u") % (*r).sort_order.get()).str()) ;
	...
	typedef sigc::signal<void>				SignalSaveXSPF ;
	typedef sigc::signal<void>				SignalLoadXSPF ;
		...
		IdV&				    m_play_queue_ref ;
		const IdV&			    m_play_history_ref ;
		...
		SignalSaveXSPF			    m_SIGNAL_save_xspf ;
		SignalLoadXSPF			    m_SIGNAL_load_xspf ;
		SignalSaveXSPF			    m_SIGNAL_save_xspf_history ;
...
*/
                        ...
                        if( event->y > m_height__headers )
			{
				...
				m_SIGNAL_track_activated.emit( track, true, false ) ;
                    ...
                    {
			m_ActionGroup->get_action("ContextRemoveFromQueue")->set_sensitive(m_model->row(d)->sort_order);
			m_ActionGroup->get_action("ContextAddToQueue")->set_sensitive(!m_model->row(d)->sort_order);
			m_ActionGroup->get_action("ContextAddToQueueNext")->set_sensitive(!m_model->row(d)->sort_order);
			...
			m_row__button_press.reset() ;
                ...
                )
                {
                    ...
                    if( m_model->m_FLAG_queue && m_row__button_press && row != m_row__button_press.get() )
                    {
			if( ModelExtents( row )) 
			{
			    DataModelFilter::RowRowMapping_sp mapping = m_model->m_mapping ;

			    DataModelFilter::RowRowMapping_t::const_iterator i1 = m_model->m_mapping->begin() ; 
			    DataModelFilter::RowRowMapping_t::const_iterator i2 = m_model->m_mapping->begin() ;
	
			    std::advance( i1, row ) ;
			    std::advance( i2, m_row__button_press.get()) ;

			    std::iter_swap( i1, i2 ) ;

			    std::swap( (**i1)->sort_order, (**i2)->sort_order ) ;

			    typedef std::vector<guint> Vec ;

			    Vec::iterator qi1 = std::find( m_play_queue_ref.begin(), m_play_queue_ref.end(), (**i1)->ID ) ;
			    Vec::iterator qi2 = std::find( m_play_queue_ref.begin(), m_play_queue_ref.end(), (**i2)->ID ) ;

			    std::iter_swap( qi1, qi2 ) ; 

			    if( m_selection && boost::get<1>(*m_selection) == *m_row__button_press)
			    {
				select_index(row) ;
			    }
		...
		set_play_on_single_tap(bool tap)
		{
		    m_play_on_single_tap = tap ;
                ...
                signal_save_xspf()
                {
                    return m_SIGNAL_save_xspf ;
                }

                SignalLoadXSPF&
                signal_load_xspf()
                {
                    return m_SIGNAL_load_xspf ;
                }

                SignalSaveXSPF&
                signal_save_xspf_history()
                {
                    return m_SIGNAL_save_xspf_history ;
                }
		    ...
		    m_SearchEntry->override_color(Util::make_rgba(1.,0.,0.,1.)) ;
                ...
                on_show_only_this_album() 
                {
                    if( m_selection )
                    {
                        ModelData_sp r = *(boost::get<0>(m_selection.get())) ;
                        MPX::Track_sp t = r->TrackSp ; 

			if( (*t).has(ATTRIBUTE_MB_ALBUM_ID))
			    _signal_0.emit( get<std::string>((*t)[ATTRIBUTE_MB_ALBUM_ID].get()));
                    }
                }
			...
			if( (*t).has(ATTRIBUTE_MB_ALBUM_ARTIST_ID))
			    _signal_1.emit( get<std::string>((*t)[ATTRIBUTE_MB_ALBUM_ARTIST_ID].get()));
                    }
                }
			...
			if( (*t).has(ATTRIBUTE_MB_ALBUM_ARTIST_ID))
			    _signal_2.emit( get<std::string>((*t)[ATTRIBUTE_MB_ALBUM_ARTIST_ID].get()));
                    }
                }
		...
		on_save_xspf()
		{
		    m_SIGNAL_save_xspf.emit() ;
		}

		void
		on_load_xspf()
		{
		    m_SIGNAL_load_xspf.emit() ;
		}

		void
		on_save_xspf_history()
		{
		    m_SIGNAL_save_xspf_history.emit() ;
		}
